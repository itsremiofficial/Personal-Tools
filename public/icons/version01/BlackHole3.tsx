import { FC } from "react";
import { IconProps, IconComponentType } from "@/types";

const IconBlackHole3: FC<IconProps> = ({
  className,
  fill = false,
  duotone = true,
  width = "1.5",
}) => {
  return (
    <>
      {!fill ? (
        <svg
          className={className}
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <circle
            cx="12"
            cy="12"
            r="2"
            stroke="currentColor"
            strokeWidth={width}
          />
          <path
            opacity={duotone ? "0.5" : "1"}
            d="M12 10C17 10 16.6 22 9 22"
            stroke="currentColor"
            strokeWidth={width}
            strokeLinecap="round"
          />
          <path
            opacity={duotone ? "0.5" : "1"}
            d="M12.3115 14C7.31152 14 7.71152 2 15.3115 2"
            stroke="currentColor"
            strokeWidth={width}
            strokeLinecap="round"
          />
          <path
            opacity={duotone ? "0.5" : "1"}
            d="M10 12.3115C10 7.31152 22 7.71152 22 15.3115"
            stroke="currentColor"
            strokeWidth={width}
            strokeLinecap="round"
          />
          <path
            opacity={duotone ? "0.5" : "1"}
            d="M14 12C14 17 2 16.6 2 9"
            stroke="currentColor"
            strokeWidth={width}
            strokeLinecap="round"
          />
        </svg>
      ) : duotone ? (
        <svg
          className={className}
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M9.10354 12.7842C9.04118 12.6764 8.98253 12.5659 8.92762 12.4529C8.30132 11.1645 8.11552 9.50716 8.34394 7.8948C8.79854 4.68585 10.9957 1.25 15.3115 1.25C15.7257 1.25 16.0615 1.58579 16.0615 2C16.0615 2.41421 15.7257 2.75 15.3115 2.75C12.0274 2.75 10.2245 5.31415 9.82911 8.1052C9.73795 8.7487 9.72706 9.38252 9.78821 9.97316C10.3366 9.37501 11.1245 9 12 9C12.9663 9 13.8258 9.45681 14.3745 10.1663C14.7911 10.5458 15.128 11.0207 15.3839 11.5471C16.0102 12.8355 16.196 14.4928 15.9676 16.1052C15.513 19.3142 13.3158 22.75 9 22.75C8.58579 22.75 8.25 22.4142 8.25 22C8.25 21.5858 8.58579 21.25 9 21.25C12.2842 21.25 14.087 18.6858 14.4824 15.8948C14.5912 15.1267 14.5857 14.3725 14.4798 13.689C13.9396 14.4804 13.0305 15 12 15C10.6145 15 9.44832 14.0607 9.10354 12.7842Z"
            fill="currentColor"
          />
          <g opacity={duotone ? "0.5" : "1"}>
            <path
              d="M13.25 12C13.25 12.8871 12.7399 13.5765 11.7971 14.0348C10.8355 14.5023 9.49284 14.679 8.1052 14.4824C5.31415 14.087 2.75 12.2842 2.75 9C2.75 8.58579 2.41421 8.25 2 8.25C1.58579 8.25 1.25 8.58579 1.25 9C1.25 13.3158 4.68585 15.513 7.8948 15.9676C9.50716 16.196 11.1645 16.0102 12.4529 15.3839C13.7601 14.7485 14.75 13.6129 14.75 12C14.75 11.5858 14.4142 11.25 14 11.25C13.5858 11.25 13.25 11.5858 13.25 12Z"
              fill="currentColor"
            />
            <path
              d="M10.75 12.3115C10.75 11.4244 11.2601 10.735 12.2029 10.2767C13.1645 9.80922 14.5072 9.63253 15.8948 9.82911C18.6858 10.2245 21.25 12.0274 21.25 15.3115C21.25 15.7257 21.5858 16.0615 22 16.0615C22.4142 16.0615 22.75 15.7257 22.75 15.3115C22.75 10.9957 19.3141 8.79854 16.1052 8.34394C14.4928 8.11552 12.8355 8.30132 11.5471 8.92762C10.2399 9.56307 9.25 10.6986 9.25 12.3115C9.25 12.7257 9.58578 13.0615 10 13.0615C10.4142 13.0615 10.75 12.7257 10.75 12.3115Z"
              fill="currentColor"
            />
          </g>
        </svg>
      ) : (
        <svg
          className={className}
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M8.92762 12.4529C9.3338 13.2885 9.9443 13.9944 10.7532 14.395C9.96022 14.5776 9.04298 14.6153 8.1052 14.4824C5.31415 14.087 2.75 12.2842 2.75 9C2.75 8.58579 2.41421 8.25 2 8.25C1.58579 8.25 1.25 8.58579 1.25 9C1.25 13.3158 4.68585 15.513 7.8948 15.9676C9.50716 16.196 11.1645 16.0102 12.4529 15.3839C13.3211 14.9619 14.0493 14.3192 14.4405 13.4626C14.583 14.2041 14.6034 15.041 14.4824 15.8948C14.087 18.6858 12.2842 21.25 9 21.25C8.58579 21.25 8.25 21.5858 8.25 22C8.25 22.4142 8.58579 22.75 9 22.75C13.3158 22.75 15.513 19.3142 15.9676 16.1052C16.196 14.4928 16.0102 12.8355 15.3839 11.5471C15.0377 10.835 14.5432 10.2171 13.9039 9.80205C14.5291 9.72593 15.2064 9.73159 15.8948 9.82911C18.6858 10.2245 21.25 12.0274 21.25 15.3115C21.25 15.7257 21.5858 16.0615 22 16.0615C22.4142 16.0615 22.75 15.7257 22.75 15.3115C22.75 10.9957 19.3141 8.79854 16.1052 8.34394C14.4928 8.11552 12.8355 8.30132 11.5471 8.92762C10.8555 9.26381 10.2527 9.73999 9.83837 10.3528C9.72613 9.6583 9.71808 8.88892 9.82911 8.1052C10.2245 5.31415 12.0274 2.75 15.3115 2.75C15.7257 2.75 16.0615 2.41421 16.0615 2C16.0615 1.58579 15.7257 1.25 15.3115 1.25C10.9957 1.25 8.79854 4.68585 8.34394 7.8948C8.11552 9.50716 8.30132 11.1645 8.92762 12.4529Z"
            fill="currentColor"
          />
        </svg>
      )}
    </>
  );
};

// Keywords for search and categorization
(IconBlackHole3 as IconComponentType).keywords = [
  "black",
  "hole",
  "3",
  "pitch-black",
  "pitch-dark",
  "bleak",
  "calamitous",
  "disastrous",
  "grim",
  "dirty",
  "soiled",
  "unclean",
  "hollow",
  "trap",
  "fix",
  "jam",
  "pickle",
  "maw",
  "yap",
  "muddle",
  "mess",
  "omega",
  "wash out",
  "cut up",
  "pull out",
  "pull up",
  "pull over",
  "pull in",
  "work out",
  "ride down",
];

export default IconBlackHole3 as IconComponentType;
